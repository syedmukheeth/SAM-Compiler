    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Unlock SAM Pro" isDarkMode={isDarkMode}>
      <div className="space-y-10 py-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="text-center space-y-4">
          ...
          <h3 className={`text-2xl font-black ${isDarkMode ? "text-sam-text" : "text-slate-900"}`}>Unlock the Full Potential</h3>
        ...
        <div className="grid gap-4">
           {[
             ...
             <div key={i} className={`flex items-center gap-6 rounded-2xl border p-5 transition-all hover:scale-[1.02] ${isDarkMode ? "border-sam-glass-border bg-sam-text/[0.02]" : "border-slate-100 bg-slate-50/50"}`}>
                ...
                <div>
                  ...
                  <p className={`text-[9px] font-bold ${isDarkMode ? "text-sam-text-muted" : "text-slate-400"}`}>{feat.desc}</p>
                </div>
             </div>
           ))}
        </div>

        <button className="w-full rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 py-5 text-[12px] font-black uppercase tracking-widest text-sam-text shadow-2xl shadow-blue-500/20 transition-all hover:brightness-110 active:scale-95">
          Upgrade for $9/mo
        </button>

        <p className={`text-center text-[9px] font-black uppercase tracking-widest ${isDarkMode ? "text-sam-text-muted" : "text-slate-300"}`}>No commitment. Cancel anytime.</p>
      </div>
    </Modal>
  );
}
